...
func TestAnthropic_VerifyValid(t *testing.T) {
	mux := http.NewServeMux()
	mux.HandleFunc("/messages", func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		w.Write([]byte(`{"content":[{"text":"0.98"}]}`))
	})

	srv := httptest.NewServer(mux)
	defer srv.Close()

	backend := NewAnthropicBackend("mock-model")
...
func TestAnthropic_FallbackGarbage(t *testing.T) {
	mux := http.NewServeMux()
	mux.HandleFunc("/messages", func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		w.Write([]byte(`{"content":[{"text":"I am a banana"}]}`))
	})

	srv := httptest.NewServer(mux)
	defer srv.Close()

	backend := NewAnthropicBackend("mock")
	backend.APIKey = "test"
	backend.BaseURL = srv.URL
